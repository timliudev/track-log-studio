...
- `feature/corner-speed` — 彎道 apex 最低速標記。
- `feature/rcnx-multisession` — RCNX 多 session 展開 + lap 資料。

### ⏳ 待放行 / 待辦
- Phase 7 直線加速測試（接彎道速度後，共用 lapMetrics/LapTable）
- 彎道偵測門檻微調（用 LogaExample 實檔；限制：只有 ARK 一尺度）
- 使用手冊同步補新功能章節（收尾更新 zh+en）
- （較後、可無人值守原型）Phase 5 合併 / 4e ECharts G-G（需加 echarts，先查最新版）
