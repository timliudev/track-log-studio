...
[`FORMAT-SUPPORT-RESEARCH.md`](./FORMAT-SUPPORT-RESEARCH.md)，本文不重複，只說明它們要怎麼接進架構。
...
| RaceChrono RCZ | ✅ `rczImporter` → `parseRcz`（**二進位**） | ⬜ | `rcz` | **第一個二進位 importer**（用了 §6 的 `parseBinary`/`headBytes` 擴充）。ZIP（fflate 解）+ `session.json`（含每圈時間/track 名）+ 每通道一個 raw 數值檔（type 0=int32／1=int64／3=float64，LE）。channel id = `k*2²⁰+lo`：`lo=5000→rc_analog_k`、`5001→rc_digital_k`、其餘查 `NAMED_LO`。GPS lat/lon 為 int32 配對 `/6e6`、速度 mm/s、heading 毫度；GPS↔ECU 以各自 int64 時間戳最近鄰對齊。真檔驗證：17791 列／147 channels／座標正確 |
| Qstarz QRacing | ⬜ 規劃中 | ⬜ | （建議 `qstarz-csv`） | 經 RaceChrono 匯出 VBO/NMEA 今天即可用；原生格式無規格。見 FORMAT-SUPPORT-RESEARCH.md |

> 對稱性現況：匯入側五個格式（loga / nmea / vbo 文字 + rcz / xrk 二進位）都已 registry 化、走同一條 worker 路徑；
> 匯出側尚未 registry 化，三個匯出器各有不同的呼叫慣例（§7）。
...
目前 `Importer.parse` **只吃 `text`**，`detect` 只拿得到 `headText`（前 4 KB 解成 UTF-8）。
這對純文字格式夠用，但二進位格式（XRK）與 ZIP 容器（RCZ）需要兩個擴充：

- **`detect` 端：`ImportCandidate` 補上 `headBytes: Uint8Array`**——二進位 magic（例如 ZIP 的
  `PK\x03\x04`、zlib 的 `0x78`）在解成文字時會被破壞，必須比對原始 bytes。
- **`parse` 端：新增 `parseBinary(bytes: Uint8Array, onProgress?)`**——並讓 worker / registry 在
  importer 提供 `parseBinary` 時改傳 `Uint8Array` 而非文字。

XRK / RCZ / Qstarz 的格式層評估（是否二進位、是否有開源 parser、瀏覽器可行性、formatId 命名建議、
接入方式）已詳列於 [`FORMAT-SUPPORT-RESEARCH.md`](./FORMAT-SUPPORT-RESEARCH.md)，此處不重複。
...
## 7. Exporter 側的已知不對稱（待清理）

匯入側已經 registry 化、對稱、走單一 worker 路徑；**匯出側還沒有**。現況誠實記錄如下：

- **`Exporter` 介面存在但未被 registry 統一驅動**（`src/domain/export/Exporter.ts`）：
  ```ts
  interface Exporter {
    readonly id: string
  ...
  ```
- **沒有 `EXPORTERS` registry、沒有 `detectExporter`**。`converterStore.ts` **直接 `new`** 各匯出器
  並各自呼叫，三者慣例不一致：
  - **`Rc3NmeaExporter`**（`rc3Nmea/Rc3NmeaExporter.ts`）雖 `implements Exporter`，但其 `export()`
    **多吃一個 `mapping: Rc3Mapping` 參數**（外加可選 `now`），**違反介面只收 `session` 的簽章**。
    store 以 `exporter.export(augmented, mapping.value)` 呼叫。
  - **VBO 根本不是 `Exporter`**：`convertToVbo(session, sourceName, now?)` 是一個 **free function**，
    且**回傳多個產物**（`VboArtifact[]`：`_ct.vbo` / `_rc.vbo` / `_channels.csv`），不符合「回傳單一
    字串」的介面。
  - **`.loga` 匯出**走的是 `patchLogaText(text, replacements)`（就地 patch 原文字），也不是 `Exporter`。
  - 因此 `converterStore` 對每種格式都有專屬分支，而非統一走 registry。

- **未來方向（提案，不在本次實作）**：把 Exporter 做成與 Importer 對稱的 registry——
  - 讓 `export()` 支援可選的格式選項物件（涵蓋 NMEA 的 `mapping`），統一簽章；
  - 讓介面能表達**多產物輸出**（涵蓋 VBO 的 `_ct`/`_rc`/`_channels`），例如回傳
    `{ name, ext, content }[]`；
  - 把 `.loga` 的就地 patch 也納為一種 exporter 類型；
  - 提供 `EXPORTERS` 陣列，讓 `converterStore` 用單一迴圈驅動，移除逐格式分支。
