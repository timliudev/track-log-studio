...
| 通用 `.csv` | ⬜ | ✅ `convertToCsv`（registry `id: 'csv'`） | — | 匯出專用（無對應 importer）。給 Race Studio 3 / Excel / Python 等任何工具匯入：`Time,GPS_Lat,GPS_Lon,GPS_Speed` 起頭，接每個其他通道（含避震衍生通道），每筆取樣一列。GPS 經緯度透過共用的 `makeFixResolver` 解析（兩種 aRacer 座標編碼皆可）；無定位的列該欄留空。數字採一般小數點、最多 4 位小數去尾零（GPS 經緯度 7 位）、換行 `\n`（LF only）、UTF-8 **不含 BOM**。細節見 `src/domain/export/csv/CsvExporter.ts` 檔頭註解。 |
