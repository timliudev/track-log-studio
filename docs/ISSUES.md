...
- [~] **B37** (a+b done, c still needs user decision) (a) `usePwaUpdate.ts` now starts an hourly `registration.update()` timer in `onRegisteredSW`, cleared in `onUnmounted`, so long-lived tabs surface the existing update toast rather than waiting on browser-default check intervals. (b) Verified via `curl -I` against the live deployment: `sw.js` and `/` already come back `Cache-Control: public, max-age=0, must-revalidate` from Cloudflare Workers static-assets defaults — no `_headers` file needed. Incidental finding (not fixed, separate perf item): hashed JS/CSS assets also come back `max-age=0` instead of long-cache/immutable — Cloudflare's immutable-asset heuristic may expect a different hash format than Vite's `name-HASH.ext`. — `f21257c`. **(c) still open: decide with user whether to ship one skipWaiting release to rescue stranded pre-toast clients.**
...
- [~] **B39** (a/b/c done, d needs user confirmation) `public/robots.txt` (allow all + sitemap pointer) and `public/sitemap.xml` (single `/` entry) added as static files, beating the SPA fallback; `index.html` head got `<link rel="canonical">` + full OG/Twitter tags, reusing the existing meta-description copy verbatim. Domain used: `track-log-studio.timliudev.workers.dev` (confirmed as the actual production URL per DESIGN.md — no custom domain exists yet). — `f21257c`. **(d) still open: no custom domain exists yet, so the workers.dev→custom-domain redirect doesn't apply currently — confirm with user if/when one is set up.** Submit sitemap to Search Console after next deploy. `llms.txt` still optional/undone.
