...
- [ ] **B38** Mobile: the bottom function-tab nav (BottomNav) still covers/blocks the page footer. Give the page/footer bottom padding equal to the nav height + `env(safe-area-inset-bottom)` (shared CSS var set by the nav), so the footer scrolls fully into view above the bar.

- [ ] **B39** Search-Console / SEO readiness (site currently has NONE of these): (a) `public/robots.txt` (allow all + sitemap pointer); (b) `public/sitemap.xml` (few routes: /, converter, analyzer, settings — or just the SPA root); (c) `index.html` head: `<meta name="description">`, `<link rel="canonical" href="https://tracklogstudio.timliudev.com/">`, Open Graph/Twitter tags (title/description/og:image can reuse pwa-512 icon); (d) decide duplicate-host handling — `track-log-studio.timliudev.workers.dev` serves the same app; 301-redirect it to the custom domain (Worker-level) or at least rely on canonical, so indexing doesn't split. NOTE: Search Console verification itself needs NO repo change — user adds the domain-property DNS TXT in Cloudflare.

- [ ] **B40** PageSpeed mobile score is 90 (desktop higher) — user wants it improved. Do NOT guess: run Lighthouse/PageSpeed against https://tracklogstudio.timliudev.com/ first and fix the actual flagged items. Likely candidates for this SPA: main-bundle size (ensure echarts/uPlot/importers are code-split & lazy — G-G chart already is), LCP waiting on JS render (consider inline critical shell/loading skeleton in index.html), font loading, unused CSS/JS. Re-measure after each change. (Search Console domain property for tracklogstudio.timliudev.com already verified by user 2026-07-13; sitemap submission waits on B39.)
