...
- [~] **B39** (a/b/c landed in `f21257c` but pointed at the WRONG domain — the agent was misled by a stale DESIGN.md §10 claiming production = workers.dev and no custom domain. Actual production = **`https://tracklogstudio.timliudev.com`**; DESIGN.md corrected by main thread.) IN PROGRESS: fix agent replacing the domain in index.html canonical/OG + robots.txt + sitemap.xml, and implementing **(d) — user APPROVED 2026-07-13 — Worker-level 301 workers.dev → custom domain** (path+query preserved). ⚠️ DO NOT push/release until this lands — the wrong canonical must never deploy. Submit sitemap to Search Console after next deploy. `llms.txt` still optional/undone.
