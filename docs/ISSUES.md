...
- [ ] **B41** A11y quick fixes from the same report (score 90, all safe/no visual change): save-slot `<select name="save-slot">` needs an accessible label (also flagged by the agentic-browsing audit); heading order jump (`<h3>Field preset` without h2 ancestor). i18n-neutral.
