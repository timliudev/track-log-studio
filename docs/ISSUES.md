...
- [ ] **B35** **Touch-capability audit (user, 2026-07-13): tablets run the FULL desktop layout but have no mouse/hover/Shift — touch-friendliness must be driven by input capability, not viewport width.** Today the codebase has ZERO `pointer: coarse`/`hover: none` media queries; all touch accommodations hang off `max-width:768px` (App.vue, BottomNav, AnalyzerView, useDashboardLayout, PwaUpdateToast), so a landscape iPad gets desktop UI with no touch support. Work items: (a) introduce a shared `useCoarsePointer()`/CSS capability queries and migrate touch affordances onto it; (b) chart pan on touch must not require Shift (B9's Shift-drag) — ensure one-finger/two-finger uPlot gestures are enabled by capability, not width; (c) hover-only affordances (tooltips, chart/map hover cursor) need tap/drag equivalents; (d) hit-target sizes ≥ ~44px for touch (⦸ toggles, lap-offset ±, resize handles, gutter drag); (e) dashboard grid drag/resize on touch (long-press drag vs scroll conflict); (f) map scrub by finger-drag (ties into B30). Audit + shared primitive first, then migrate screen by screen.
