...
- **觸控友善四層政策（2026-07-13 拍板，B35；取代「以寬度判斷」，也不可「以裝置分類判斷」——同一台機器會換輸入方式：DeX/手機接藍牙鍵鼠、S Pen、平板純觸控、筆電觸控板皆為實際情境）**：
  ...
  - **實作註記（B35 基礎建設，2026-07-13）**：`composables/useInputCapabilities.ts` 是唯一 primitive，包三條 `matchMedia`（`any-pointer: coarse`／`pointer: coarse`／`any-hover: none`，各自掛 change 監聽）+ `settingsStore.inputModePref`（層4覆蓋，'auto' 時原樣透傳、'touch'/'pointer' 時三個能力全部釘死）；在 App.vue 掛一次（同 useTheme/useLocale 模式），並鏡射成 `<html data-any-pointer-coarse|data-pointer-coarse|data-any-hover-none>` 三個 boolean attribute，讓純 CSS 用 `:root[data-any-pointer-coarse] .foo {...}` 就能吃到訊號，不必每個元件都 import 這個 composable。已遷移到能力訊號的點擊目標：`LapTable.vue` 的 ⦸ 排除鈕、`SessionLapComparison.vue` 的 offset ±／reset 鈕、`UPlotChart.vue` 的重設縮放鈕、`DashboardCard.vue` 的收折/釘選鈕、`TimeSeriesChart.vue`/`ScatterChart.vue` 的刪除圖表鈕、`AnalyzerView.vue` 的格線縮放把手（`.vgl-item__resizer`，B110）。**B110 更新**：grid-layout-plus 的 resize 把手 30px 尺寸原本只掛在 `AnalyzerView.vue` 自己的 `@media (max-width: 768px)`（純寬度斷點），期間 `theme.css` 曾補一條全域、unscoped 規則從外部同樣把 `.analyzer .vgl-item__resizer` 在寬螢幕+粗指標下長到 30px 當作暫時繞道；B110 已直接把 `AnalyzerView.vue` 自己那份規則改成 `:root[data-any-pointer-coarse] .analyzer {...}`（不再用寬度判斷），根本問題解決後 `theme.css` 那條暫時繞道的重複規則已移除（只留下說明性註解），不再有兩份互相疊加的拷貝。真正屬於「版面」的寬度斷點（`BottomNav` 顯示切換、dashboard 單欄堆疊——`useDashboardLayout.ts` 的 `MOBILE_BREAKPOINT_PX`）維持寬度判斷不變,那些跟輸入能力無關。圖表手勢部分：`UPlotChart.vue` 觸控拖曳平移/雙指縮放本來就是逐事件 `pointerType`,並非寬度 gate；本輪修正的是 `pen` 誤入觸控分支的 bug（抽成可測的 `isTouchGesturePointer()`),改為與 `mouse` 同路（框選縮放 + Shift 平移),理由是 S Pen 已有懸浮能力,自然比照滑鼠體驗。
  ...
  - **實作註記（圖表觸控與固定中線重寫，2026-07-17 `e0d02d2`）**：固定中線約束在 plot 矩形內、
    resize/scale 後重新量測；固定模式支援滾輪/雙指縮放與平移。觸控在圖表上走 450ms pending
    狀態機（`domain/analysis/chartPointerGesture.ts`）：靜止長按=選值顯示、水平移動=平移、
    垂直移動=交還原生頁面捲動、第二指=雙指縮放。
  - **實作註記（B59/B64 手機卡片，2026-07-16/17）**：手機單欄下 grid resize 以 per-item
    `resizeOption` 鎖 `bottom`-only（庫的角把手原生綁三邊、拖曳中寬度跟手指走）；釘選卡改
    全寬貼齊 + 迷你/展開切換（session 內記憶）。
...
### 設計決策來源紀錄（2026-06-18 / 06-23 實機回饋，均已落地）
...
- **時間帶 / Sector 完整性過濾（#1 延伸）**：`lapValidity` + 時間/距離帶（出處 `auto`/`user`，見 B58 語意——
  auto 於每次圈重算時重新建議、user 永不被覆蓋、清除=重新武裝、主檔切換豁免一次）；
  `sectorValidity.ts` `invalidSectorLapIndices` 逐 gate 幾何判定，濾掉切西瓜假圈。
- **圈次分析（E）**：`sectorTiming.ts` `computeSectorTimes` + `computeOptimalLap`（逐 sector 取最小組成理論最佳圈）；
  `gateOrder.ts` `sortGatesByPosition` 依圈上位置排序 gate；理論最佳圈摘要固定在 `SectorPanel` header（B47）。
- **軸顯示（#5/#6）**：X 軸可讀格式（`axisFormat.ts`）；未選圈且有絕對起點時疊加第二條時鐘 X 軸，
  起點由 `startTime.ts` `sessionStartAnchor` 決定（GPS_UTC 優先 → `meta.createdDate` → null），時區走 `settingsStore.tzOverride`。
- **軌跡熱力上色（#10/#11）**、**圖表疊圈（C2，#4/#7）**、**版面 4e（#12/#13/#8）**、**本機持久化（D）**：均已落地，細節見對應 store/composable。
- **Track 獨立匯入 + 雲端同步**：把 #8 D「雲端同步延後」往前推並具體化——普世賽道 → GitHub 共享庫、
  個人設定/圈速 → 個人雲端備份；完整設計見 [`CLOUD-TRACK-DESIGN.md`](./CLOUD-TRACK-DESIGN.md)（設計中，尚未實作）。
- **格式對稱**：匯入/匯出/`LogSession` 架構見 [`ARCHITECTURE-FORMATS.md`](./ARCHITECTURE-FORMATS.md)、
  進度見 [`IMPORT-FORMATS-STATUS.md`](./IMPORT-FORMATS-STATUS.md)、格式研究見 [`specs/`](./specs/)。
...
> **短期、具體、可執行的 bug/請求一律以 [`ISSUES.md`](./ISSUES.md) 為唯一真實來源**
> （逐項狀態＋修復 commit）。本節**不再重列**那些條目；只保留兩種東西：
> (1) 尚在「要不要做／怎麼做」方向階段、還沒細到能拆成 ISSUES 條目的**較長期項目**；
> (2) **已完成大項**各一行，當作設計層級的粗粒度變更史。
>
> 規則：任何較長期項目一旦拍板、拆成可執行條目，就搬進 ISSUES.md 追蹤到打勾，
> 並從本節移除。實作細節與格式規格研究見 [`docs/specs/`](./specs/)。
