...
- ~~**快速篩選無用圈（時間帶過濾）**：設一個有效圈速區間，超出者自動標記無用圈剔除。
  ...
  **語意更新（B58，2026-07-16/17）**：時間與距離區間各帶「出處」標記 `auto`/`user` —
  自動建議寫入=auto、使用者輸入=user。auto 區間在**每次圈重算**（拉起終點線、換圈來源、
  換場地）時重新建議（預設線的初始垃圾微圈沒有可靠的一次性建議時機，只能持續刷新直到
  使用者接手）；user 區間永不被自動覆蓋；「清除區間」把出處歸零=重新武裝，下次圈變更
  重新建議；主檔切換（`primarySwapPending`）豁免一次刷新、區間視為全域共用不重置。
