...
> **#9 單圈 GNSS 偏移微調**其實已落地(PRIMARY 場):`MapAlignPanel.vue`(地圖對齊卡,選 ≥2 圈時顯示)提供逐圈東/北 0.5m 微調 + reset,存 `lapStore.offsets[i].mapX/mapY`,經 `AnalyzerView` 的 `highlightLaps[].offset` 由 TrackMap 繪製——此段先前過期(2026-07-23 更正)。唯一殘留為 niche:比較場(`sessionLapOffsets`)只接了 time/dist、未接 per-lap mapX/mapY。**F 行動裝置真機驗收** = F1,phases 1-5 已實作(見 [`ISSUES.md`](./ISSUES.md) F1),待真機驗證。
...
- **§11b 待辦佇列殘項**：#9 單圈 GNSS 偏移微調 → PRIMARY 場已落地(`MapAlignPanel.vue`,見上方更正),僅剩比較場 per-lap mapX/mapY 未接(niche);F 行動裝置真機驗收 = F1 phases 1-5 已實作、待真機驗證（診斷面板已備 `?debug=1`）。
