...
- **⚠ 衝突處理**：`lapColor`（圈次身分色，`order` 為 key）與
  ...
  圈」），避免新增第二組獨立色盤搶佔同一組視覺資源。這一點在 Phase 2
  （XY 散佈圖）就會第一次真正遇到，需要在該階段的實作前明確定案。
- **圖例呈現**：單一、全域、可收合的比較面板（§3.4），是所有卡片共用的
  唯一圖例來源；各張圖表卡片本身**不重複畫色塊清單**，只在資料上套用
...
引用 `docs/journal/PERF-AUDIT-2026-07-08.md`（2026-07-08 完整量測，發布於
`develop`，大檔基準：39.16MB / 57,594 rows / 272 channels）：
...
- `src/domain/analysis/downsample.ts` — 已寫好但從未接線的 `lttb()`，
  Phase 1 的效能安全網
- `docs/journal/PERF-AUDIT-2026-07-08.md` — 本文件 §6 引用的完整效能
  量測基準
