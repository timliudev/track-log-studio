/**
 * B102a — F1 phase 5 手勢引擎:行動版單欄 grid 儀表板是整個「頁面」在捲動一
 * 個內容高度撐出來的長 grid——沒有自己的內部捲動容器,捲的是 `window`/
 * document 本身。拖曳卡片排序時,手指若停在畫面頂/底邊緣附近,應該要能把頁
 * 面往那個方向捲,才搆得到目前畫面外的位置——沒有這個機制,一次拖曳能到達
 * 的範圍就被死死限制在「手指按下那一刻螢幕裝得下的範圍」。
 *
